...
(function () {
    ...
    function resizable(root, state) { const handle = part(root, "handle") || q(root, "[data-ui-resize-handle]"); const pane = parts(root, "panel pane")[0] || q(root, "[data-ui-resize-pane]"); const group = part(root, "group") || root; if (!handle || !pane) return; const valueTextPrefix = handle.dataset.uiAriaValuetextPrefix?.trim(), min = Number(handle.getAttribute("aria-valuemin") || 0), max = Number(handle.getAttribute("aria-valuemax") || 100); let targetValue = Number(handle.getAttribute("aria-valuenow")); const announce = (value) => { handle.setAttribute("aria-valuenow", String(value)); handle.setAttribute("aria-valuetext", valueTextPrefix ? `${valueTextPrefix}: ${value} percent` : `${value} percent`); }; const sync = () => { const container = group.getBoundingClientRect().width || 1, rendered = pane.getBoundingClientRect().width || 0; announce(Math.max(min, Math.min(max, Math.round(rendered / container * 100)))); }; const setWidth = (width) => { const container = group.getBoundingClientRect().width || 1; width = Math.max(container * min / 100, Math.min(container * max / 100, width)); targetValue = Math.round(width / container * 100); pane.style.flexBasis = `${width}px`; pane.style.flexShrink = "0"; announce(targetValue); window.requestAnimationFrame?.(sync); }; if (Number.isFinite(targetValue)) setWidth(group.getBoundingClientRect().width * targetValue / 100); add(state, window, "resize", () => setWidth(group.getBoundingClientRect().width * targetValue / 100)); add(state, handle, "pointerdown", (event) => { const startX = event.clientX, startWidth = pane.getBoundingClientRect().width; handle.setPointerCapture?.(event.pointerId); const finish = () => { removeMove(); removeUp(); removeCancel(); if (handle.hasPointerCapture?.(event.pointerId)) handle.releasePointerCapture(event.pointerId); }; const removeMove = add(state, handle, "pointermove", (next) => setWidth(startWidth + next.clientX - startX)); const removeUp = add(state, handle, "pointerup", finish, { once: true }); const removeCancel = add(state, handle, "pointercancel", finish, { once: true }); }); add(state, handle, "keydown", (event) => { if (event.key === "ArrowLeft" || event.key === "ArrowRight") { event.preventDefault(); setWidth(pane.getBoundingClientRect().width + (event.key === "ArrowLeft" ? -16 : 16)); } }); }
    function extras(root, state, component) {
        const toggles = component === "toolbar" ? [] : component === "toggle" || component === "toggle-group" ? parts(root, "trigger") : qa(root, "[data-ui-toggle]"); toggles.forEach((button) => add(state, button, "click", () => { if (optionDisabled(button)) return; const group = component === "toggle-group" ? root : button.closest("[data-ui-toggle-group]"); const pressed = button.getAttribute("aria-pressed") !== "true"; if (group && group.dataset.multiple !== "true") qa(group, "[aria-pressed]").filter((item) => !optionDisabled(item)).forEach((item) => item.setAttribute("aria-pressed", "false")); button.setAttribute("aria-pressed", String(pressed)); }));
        ...
        if (component === "number-field") { const input = part(root, "input") || q(root, "input[type=number]"), decrement = part(root, "decrement"), increment = part(root, "increment"); if (input) { const numeric = (name, fallback) => { const attribute = input.getAttribute(name); if (attribute === null || attribute === "") return fallback; const value = Number(attribute); return Number.isFinite(value) ? value : fallback; }; const sync = () => { const value = Number(input.value); if (decrement) decrement.disabled = Number.isFinite(value) && value <= numeric("min", -Infinity); if (increment) increment.disabled = Number.isFinite(value) && value >= numeric("max", Infinity); }; const change = (direction) => { const step = numeric("step", 1) || 1, minimum = numeric("min", -Infinity), maximum = numeric("max", Infinity), current = Number(input.value); const origin = Number.isFinite(current) ? current : Number.isFinite(minimum) ? minimum : 0; input.value = String(Math.max(minimum, Math.min(maximum, origin + direction * step))); input.dispatchEvent(new CustomEvent("input", { bubbles: true })); input.dispatchEvent(new CustomEvent("change", { bubbles: true })); input.focus(); }; if (decrement) add(state, decrement, "click", () => change(-1)); if (increment) add(state, increment, "click", () => change(1)); add(state, input, "input", sync); add(state, input, "change", sync); sync(); } }
        if (component === "toolbar") { const controls = () => qa(root, "button,a[href],[role=button]").filter((item) => !optionDisabled(item)); const rove = (target) => controls().forEach((item) => { item.tabIndex = item === target ? 0 : -1; }); const initial = controls(); if (initial.length) rove(initial[0]); add(state, root, "focusin", (event) => { if (controls().includes(event.target)) rove(event.target); }); add(state, root, "keydown", (event) => { const keys = ["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "Home", "End"]; if (!keys.includes(event.key)) return; const available = controls(), index = available.indexOf(document.activeElement); if (index < 0 || !available.length) return; const vertical = root.getAttribute("aria-orientation") === "vertical", previous = vertical ? "ArrowUp" : "ArrowLeft", next = vertical ? "ArrowDown" : "ArrowRight"; const target = event.key === "Home" ? available[0] : event.key === "End" ? available.at(-1) : event.key === previous ? available[(index - 1 + available.length) % available.length] : event.key === next ? available[(index + 1) % available.length] : null; if (target) { event.preventDefault(); rove(target); target.focus(); } }); }
        ...
        if (component === "data-table" || root.matches("[data-ui-table]")) { const table = q(root, "table") || root, headers = qa(table, "th[data-ui-part=sort],th[aria-sort]"), status = part(root, "status"), empty = part(root, "empty"), initialStatus = status?.textContent.trim() || "", statusSuffix = initialStatus.replace(/^\d+\s+/, "").trim() || "matching results"; const rows = () => qa(table, "tbody tr"); const syncResults = () => { const visible = rows().filter((row) => !row.hidden).length; if (empty) empty.hidden = visible !== 0; if (status) status.textContent = `${visible} ${statusSuffix}`; }; headers.forEach((cell) => { const explicitTrigger = q(cell, "[data-ui-table-sort-trigger]"), trigger = explicitTrigger || (q(cell, "button,[role=button]") ? null : cell); if (!trigger) return; add(state, trigger, "click", (event) => { event.preventDefault(); const index = Array.from(cell.parentElement.children).indexOf(cell), ascending = cell.getAttribute("aria-sort") !== "ascending"; headers.forEach((item) => item.setAttribute("aria-sort", item === cell ? (ascending ? "ascending" : "descending") : "none")); rows().sort((a, b) => a.children[index].textContent.localeCompare(b.children[index].textContent, undefined, { numeric: true }) * (ascending ? 1 : -1)).forEach((row) => table.tBodies[0]?.append(row)); syncResults(); }); }); qa(root, "[data-ui-filter],[data-ui-table-filter]").forEach((input) => add(state, input, "input", () => { rows().forEach((row) => { row.hidden = !row.textContent.toLowerCase().includes(input.value.toLowerCase()); }); syncResults(); })); syncResults(); }
