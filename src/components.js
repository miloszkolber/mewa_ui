...
(function () {
    ...
    function modal(root, state) {
        ...
        panels.forEach((panel) => {
            ...
            qa(panel, "form[data-ui-dialog-form]").forEach((form) => add(state, form, "submit", (event) => { if (!form.checkValidity()) return; event.preventDefault(); emit(root, "core-ui:dialog-submit", { form }); close(panel); }));
    ...
    function choices(root, state) {
        ...
        const setCommitted = (item) => { if (!item || optionDisabled(item)) return; committed = item; allItems().forEach((entry) => entry.setAttribute("aria-selected", String(entry === item))); input.value = value(item); if (submittedValue) submittedValue.value = input.value; setActive(item); input.setAttribute("aria-expanded", "false"); if (list) list.hidden = true; emit(root, "core-ui:select", { value: input.value, item }); };
    ...
    function extras(root, state, component) {
        ...
        if (component === "questionnaire" || root.matches("[data-ui-questionnaire]")) { const steps = qa(root, "[data-ui-question],[data-ui-part=step]"); if (!steps.length) return; let index = Math.max(0, steps.findIndex((step) => !step.hidden)); const nextButtons = qa(root, "[data-ui-question-next],[data-ui-part=next]"), previous = qa(root, "[data-ui-question-prev],[data-ui-part=previous],[data-ui-part=prev]"), submit = part(root, "submit"); const update = (next, focusStep = false) => { index = Math.max(0, Math.min(next, steps.length - 1)); steps.forEach((step, item) => { step.hidden = item !== index; }); const final = index === steps.length - 1; nextButtons.forEach((button) => { button.hidden = final; }); previous.forEach((button) => { button.hidden = index === 0; button.disabled = index === 0; }); if (submit) submit.hidden = !final; const progress = part(root, "progress"); if (progress) progress.textContent = `Step ${index + 1} of ${steps.length}`; if (focusStep) { const step = steps[index]; if (step.getAttribute("tabindex") === null) step.setAttribute("tabindex", "-1"); step.focus(); } emit(root, "core-ui:question-change", { step: index, question: steps[index] }); }; update(index); nextButtons.forEach((button) => add(state, button, "click", (event) => { const invalid = qa(steps[index], "[required]").find((field) => !field.checkValidity()); if (invalid) { event.preventDefault(); invalid.reportValidity(); return; } event.preventDefault(); update(index + 1, true); })); previous.forEach((button) => add(state, button, "click", (event) => { event.preventDefault(); update(index - 1, true); })); }
