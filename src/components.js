...
(function () {
    ...
    function navbarVertical(root, state) { const trigger = part(root, "trigger"), panel = part(root, "panel"); if (!trigger || !panel) return; const compact = () => window.matchMedia?.("(max-width: 48rem)")?.matches === true; let wasCompact = compact(); if (wasCompact) open(trigger, panel, false); add(state, trigger, "click", () => open(trigger, panel, panel.hidden)); add(state, panel, "click", (event) => { if (compact() && event.target.closest?.("a[href]")) open(trigger, panel, false); }); add(state, root, "keydown", (event) => { if (compact() && event.key === "Escape" && !panel.hidden) { event.preventDefault(); open(trigger, panel, false); trigger.focus(); } }); add(state, document, "pointerdown", (event) => { if (compact() && !panel.hidden && !root.contains(event.target)) open(trigger, panel, false); }); add(state, window, "resize", () => { const nextCompact = compact(); if (nextCompact === wasCompact) return; wasCompact = nextCompact; open(trigger, panel, !nextCompact); }); }
