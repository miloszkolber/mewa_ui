...
const { argbFromLstar, blueFromArgb, greenFromArgb, lstarFromArgb, redFromArgb } = colorUtils;
...
function renderPaletteBlock() {
  ...
  lines.push(
    '',
    '    /* Tuned chromatic alpha ramps retain their saturation and gain transparent endpoints. */'
  );
...
if (check) {
  if (actual !== expected) {
    console.error(
      `FAIL ${path.relative(projectRoot, basePath)} is not generated from the HCT palette recipe`
    );
    process.exitCode = 1;
  } else {
