...
Button has one size. The height is `var(--size-900)`, which is 36px.

Use independent attributes for shape and content. Add the boolean `data-icon-only` attribute for a square icon-only button. Keep an accessible name via `aria-label`.

```html
<button class="btn" type="button" data-variant="secondary" data-icon-only aria-label="Open settings">
  <svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M12 2 4 9v11h5v-6h6v6h5V9z"/></svg>
</button>
```

Do not add a compact size. Keep the 36px target in dense rows too.
...
Keep the 36px target for every action.
