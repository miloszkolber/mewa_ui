...
Keep a visible `.todo-item-status` label inside every task item.

Update the status label and decorative mark whenever application code changes `data-status`.

Use `To do`, `In progress`, `Done`, and `Error` for the corresponding status labels.

Use `data-todo-progress` when the optional module should derive the summary count.

## Behavior

The module counts direct `[data-todo-item]` or `.todo-item` elements inside `.todo-list-items`.
...
The module does not set item status, item labels, or marks.
