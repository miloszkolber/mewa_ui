...
<form class="composer" action="/messages" method="post" data-submit-on="mod-enter">
  ...
  <div class="composer-actions">
    <div class="composer-actions-leading">
      <button class="btn" type="button" data-variant="ghost" data-icon-only aria-label="Attach file">
...
Omit `data-submit-on` to keep keyboard submission shortcuts off, even when the module is loaded for another Composer.
...
Keyboard submission uses the first authored submit button and preserves its submitter value and form overrides.

Keyboard submission does not activate a disabled submit button.
...
Use native `readonly` when the message must remain readable and selectable but cannot be edited.
