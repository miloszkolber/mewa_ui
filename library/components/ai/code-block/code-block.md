...
The module keeps the copy control hidden when the Clipboard API is unavailable.
...
Application code owns `.code-block-state` text and keeps that text synchronized with `data-streaming`.
