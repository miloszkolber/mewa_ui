...
export function enhance(scope) {
  ...
  queryAll(scope, rootSelector).forEach((root) => {
    ...
    lifecycle.listen(root, root, 'input', (event) => {
      ...
      if (event.isComposing || !target.matches?.('[data-table-filter], .data-table-filter')) return;
      currentPage = 1;
      emitFilter();
    });

    lifecycle.listen(root, root, 'compositionend', (event) => {
      if (!event.target.matches?.('[data-table-filter], .data-table-filter')) return;
      currentPage = 1;
      emitFilter();
    });
