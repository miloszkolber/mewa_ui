...
The module preserves authored `aria-label` and `aria-labelledby` values on slides.
...
Arrow Left and Arrow Right move one slide when the carousel root has focus.

Home and End move to the first and last slide when the carousel root has focus.

Keys keep their native behavior inside interactive or independently focusable slide content.
