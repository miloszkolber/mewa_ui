...
When an activated move button becomes disabled at a boundary, focus moves to the reordered item.
...
Arrow-key management applies only while a list item itself has focus.

Nested editable controls keep their native keys.
