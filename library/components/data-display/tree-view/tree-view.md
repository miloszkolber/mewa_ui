...
Tree View also includes the structural `.tree-indicator` subprimitive. The Figma-aligned variants are `default`, `first`, `last`, `overflowTop`, and `overflowBottom`. The kebab-case `overflow-top` and `overflow-bottom` values are accepted too. The older `line`, `branch`, and `overflow` hooks retain their existing connector shapes for compatibility. Tree Indicator is not a separate registry component.
...
Set `aria-disabled="true"` on an unavailable `.tree-item`.

Managed navigation skips disabled items and their descendants.

The module prevents activation of disabled branch triggers and leaf controls.

Use `data-state="disabled"` only for a visual specimen, not as an interaction contract.
