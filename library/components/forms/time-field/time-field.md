...
Numeric segments report each supported editing event. The period select reports one component event after its native `change` event, not a second event for native `input`.
