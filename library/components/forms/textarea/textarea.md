...
Use `readonly` when users must still focus, select, copy, and submit the value. Readonly styling does not require a Field wrapper.

Use `disabled` when the control is unavailable and must not submit. Keep resizing available for readonly content.
