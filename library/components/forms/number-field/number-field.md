...
The module dispatches no events when the native value does not change or when `step="any"` prevents native stepping.

The module disables step buttons while the input is disabled or readonly. Keep application-owned button availability in the initial markup. Call the controller update method after changing an ancestor fieldset state.
...
The stylesheet hides step buttons until successful enhancement. Native spin controls remain available without the module.
