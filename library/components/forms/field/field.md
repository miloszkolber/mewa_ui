...
Readonly presentation uses the native `readonly` attribute on an input or textarea. Do not infer readonly state from `:read-only` on a select; an enabled select remains editable.
