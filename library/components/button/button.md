...
Size and content are independent. Add the boolean `data-icon-only` attribute
for square icon-only buttons; combine it with `data-size="sm"` for a compact
32px square. Keep an accessible name via `aria-label`. The old `icon` and
`icon-sm` size values are replaced by this independent attribute.
