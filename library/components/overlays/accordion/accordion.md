...
Ready Accordion triggers use a 44px minimum height, 12px horizontal padding, an 8px icon gap, and 14px bold-relaxed text with `MONO:0`. Focus is explicit; hover does not add an underline.

Native summary elements do not support `disabled`. Do not use a disabled-looking state as a substitute for preventing activation. Keep unavailable information visible instead of adding a nonfunctional disclosure.
