...
function initHoverCard(trigger) {
  ...
  installGlobalListeners(trigger.ownerDocument);
  let owned = documentTriggers.get(trigger.ownerDocument);
  if (!owned) documentTriggers.set(trigger.ownerDocument, (owned = new Set()));
  owned.add(trigger);
