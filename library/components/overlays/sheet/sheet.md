...
The component uses the shared spatial motion duration for edge disclosure.
...
The browser owns initial focus.

Set `autofocus` on the control or focusable heading that should receive initial focus.

Use `tabindex="-1"` with `autofocus` on a heading when the user should read context before reaching a control.
...
Control state feedback uses the fast motion primitives.

The sheet uses the spatial motion duration to slide from its attached edge while the backdrop fades.
