...
A Tooltip opens immediately while another Tooltip in the same document was recently visible.
...
Leaving or blurring the trigger closes the Tooltip when neither focus nor the pointer remains on the trigger or hint.

Escape closes the Tooltip and cancels a pending open. Destroying the last enhanced trigger releases the document listener and shared delay timer.
