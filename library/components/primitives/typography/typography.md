...
Fenced `pre` and `code` keep their treatment when the renderer adds a language class such as `language-js`. Lists with `task-list` or `contains-task-list` and items with `task-list-item` keep the document spacing. Mark read-only task checkboxes `disabled` in renderer output. CSS does not disable interactive controls.
