...
Visual state hooks do not disable native behavior. Remove `href` from an unavailable route and expose `aria-disabled="true"`. Use the native `disabled` attribute on an unavailable search input. Show a shortcut only when the application implements it.
...
Nav route lists support horizontal and vertical orientation, not collapsed rows. Keep route names visible. Use Sidebar when navigation requires a documented collapse interaction; the Nav Brand subprimitive has its own collapsed-label treatment.
