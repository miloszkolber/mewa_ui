...
Apply `.btn` to action links and buttons. Load Button styles for this composition. App Shell action regions own layout, not control appearance.
...
<ul class="app-status-list" aria-label="Worker status">
  <li class="app-status-row">
    ...
    <div class="app-status-actions">
      <button class="btn" type="button" data-variant="secondary">View run</button>
    </div>
  </li>
</ul>
```
  ...
  <ul class="app-dense-list" aria-label="Feeds">
    <li class="app-dense-row">
      ...
      <div class="app-dense-actions">
        <button class="btn" type="button" data-variant="ghost">Refresh</button>
      </div>
    </li>
  </ul>
</section>
...
Use the same 36px Button size for row actions. Button has no compact size variant.

Use `data-variant="secondary"` for ordinary secondary actions.
...
```html
<script type="module" src="/vendor/mewa-ui/auto/app-shell.js"></script>
```
