...
(function () {
    ...
    function popup(root, state, type) {
        ...
        pairs.forEach((pair) => {
            ...
            if (type === "tooltip" || type === "hover-card") { add(state, pair.trigger, "focus", () => show(pair)); add(state, pair.trigger, "blur", () => open(pair.trigger, pair.panel, false)); add(state, pair.panel, "pointerenter", () => show(pair)); add(state, pair.panel, "pointerleave", () => open(pair.trigger, pair.panel, false)); }
    ...
    function choices(root, state) {
        ...
        add(state, input, "input", () => { show(); const needle = input.value.toLowerCase(); allItems().forEach((item) => { item.hidden = !item.textContent.toLowerCase().includes(needle); }); const visible = items(); if (visible.length) setActive(visible[0]); else input.removeAttribute("aria-activedescendant"); });
    ...
    function extras(root, state, component) {
        ...
        if (component === "questionnaire" || root.matches("[data-ui-questionnaire]")) { const steps = qa(root, "[data-ui-question],[data-ui-part=step]"); let index = Math.max(0, steps.findIndex((step) => !step.hidden)); const nextButtons = qa(root, "[data-ui-question-next],[data-ui-part=next]"), previous = qa(root, "[data-ui-question-prev],[data-ui-part=previous],[data-ui-part=prev]"), submit = part(root, "submit"); const update = (next) => { index = Math.max(0, Math.min(next, steps.length - 1)); steps.forEach((step, item) => { step.hidden = item !== index; }); const final = index === steps.length - 1; nextButtons.forEach((button) => { button.hidden = final; }); previous.forEach((button) => { button.hidden = index === 0; }); if (submit) submit.hidden = !final; const progress = part(root, "progress"); if (progress) progress.textContent = `Step ${index + 1} of ${steps.length}`; emit(root, "core-ui:question-change", { step: index, question: steps[index] }); }; nextButtons.forEach((button) => add(state, button, "click", (event) => { const invalid = qa(steps[index], "[required]").find((field) => !field.checkValidity()); if (invalid) { event.preventDefault(); invalid.reportValidity(); return; } update(index + 1); })); previous.forEach((button) => add(state, button, "click", () => update(index - 1))); update(index); }
