...
(function () {
    ...
    function extras(root, state, component) {
        ...
        const sliderInputs = qa(root, "input[type=range]"); qa(root, "output[for],[data-ui-slider-output]").forEach((output) => { const input = document.getElementById(output.htmlFor) || sliderInputs[0]; if (input && input.getAttribute("type") === "range") { const update = () => { const maximum = input.getAttribute("max") || input.max; const text = `${input.value}${output.dataset.unit || (maximum === "100" ? "%" : "")}`; output.textContent = text; input.setAttribute("aria-valuetext", output.dataset.ariaText || (maximum === "100" ? `${input.value} percent` : text)); }; update(); add(state, input, "input", update); } }); sliderInputs.filter((input) => !qa(root, "output[for],[data-ui-slider-output]").some((output) => (document.getElementById(output.htmlFor) || sliderInputs[0]) === input)).forEach((input) => { const update = () => input.setAttribute("aria-valuetext", (input.getAttribute("max") || input.max) === "100" ? `${input.value} percent` : input.value); update(); add(state, input, "input", update); });
