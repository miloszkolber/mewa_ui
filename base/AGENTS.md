...
You are working on the **base/** design system, a fork of
[shadcn-html](https://github.com/codylindley/shadcn-html) at upstream commit
`0964e09e` (v0.7.13-alpha), MIT. The consumer-facing system lives in `base/`.
...
```
base/
├── theme/default-semantic-tokens.css          ← design tokens (source of truth for colors, spacing, tracking)
...
├── documentation/                 ← reference implementations + doc site
│   ├── *.html                     ← one page per component + overview pages
│   ├── css/docs-utilities.css    ← hand-written utility classes for doc pages
│   ├── css/docs-theme.css         ← doc-site font overrides (Geist sans/mono, not part of the system)
...
Each component at `base/components/{name}/` contains:
...
`base/theme/default-semantic-tokens.css` defines all CSS custom properties. These must match
...
The doc site is fully static. Serve `base/documentation/` with any static
...
1. **Create the component folder** → `base/components/{name}/`

2. **Write the component skill** → `base/components/{name}/component-skill.md`
...
5. **Create the doc page** → `base/documentation/{name}.html`
...
6. **Update layout.js** → add the component to the `NAV` array and `BUILT` set
   in `base/documentation/js/layout.js` (this is the single source of truth for sidebar nav)
