...
function verifyChecksums(base) {
  ...
  files.forEach((filename) => {
    const digest = crypto.createHash("sha256").update(fs.readFileSync(path.join(base, filename))).digest("hex");
...
function assertLocalPath(base, owner, relativePath) {
  ...
  assert(relativePath.startsWith("./") || !relativePath.startsWith("."), `${owner}: malformed path ${relativePath}`);
  const filename = path.resolve(base, relativePath);
  const relative = path.relative(base, filename);
  assert(relative !== ".." && !relative.startsWith(`..${path.sep}`) && !path.isAbsolute(relative), `${owner}: path escapes package`);
...
await test("the core package is private, versioned, dependency-free, and export-mapped", () => {
  ...
  assert.equal(workspacePackage.private, true, "the source workspace must remain blocked from npm publication");
  assert.match(workspacePackage.version, /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/, "release version must be tag-safe semver");
  assert.equal(packaged.name, "mewa-ui");
  assert.equal(packaged.version, workspacePackage.version);
  assert.equal(packaged.private, true, "GitHub-only releases must stay blocked from npm publication");
...
await test("the package manifest exposes every component and its dependencies", () => {
  ...
  registry.components.forEach((component) => {
    ...
    assert(fs.existsSync(path.join(coreRoot, packaged.css)), `${component.slug}: missing CSS entry`);
    assert(fs.existsSync(path.join(coreRoot, `css/components/${component.slug}.css`)), `${component.slug}: missing source CSS copy`);
    ...
    [packaged.css, packaged.controller, packaged.component, packaged.auto]
      .filter(Boolean)
      .forEach((relativePath) => assertLocalPath(coreRoot, `${component.slug} manifest`, relativePath));
  });

  Object.values(manifest.foundations).flat().forEach((relativePath) => {
...
await test("controller modules are side-effect-free and safe to import without a DOM", async () => {
  for (const component of enhanced) {
    ...
    assert(!source.includes("mewa:auto:"), `${component.slug}: auto marker escaped into controller`);
    assert(!source.includes("registerBehavior"), `${component.slug}: controller has an automatic side effect`);
    ...
    const types = fs.readFileSync(path.join(coreRoot, `controllers/${component.slug}.d.ts`), "utf8");
    assert.equal(/declare function destroy\b/.test(types), typeof module.destroy === "function", `${component.slug}: destroy type drift`);
  }
});
...
await test("component controllers compose declared behavior dependencies without side effects", async () => {
  for (const component of enhanced) {
    ...
    const source = fs.readFileSync(filename, "utf8");
    assert(!source.includes("registerBehavior"), `${component.slug}: component controller has an automatic side effect`);
    const module = await import(`${pathToFileURL(filename).href}?contract=component-${component.slug}`);
    const expected = [...dependencyClosure(component, "behaviorDependencies"), component.slug]
      .filter((slug) => registry.components.find((entry) => entry.slug === slug).jsMode !== "none");
    assert.deepEqual(module.behaviors.map((entry) => entry.name), expected, `${component.slug}: behavior closure drift`);
    ...
    assert.deepEqual(module.enhance(), [], `${component.slug}: component entry is not safe without a DOM`);
    module.destroy();
  }
});
...
await test("the shared enhancer observes one root once for every registered behavior", async () => {
  const runtime = await import(`${pathToFileURL(path.join(coreRoot, "runtime/enhancer.js")).href}?contract=lifecycle`);
  ...
  try {
    const first = { name: "first", enhance: (root) => calls.push(["first", root]), destroy: (root) => calls.push(["first:destroy", root]) };
    const second = { name: "second", enhance: (root) => calls.push(["second", root]) };
    const enhancer = runtime.createEnhancer([first, second]);
    const observedRoot = { id: "document" };
...
await test("an obsolete observer disposer cannot disconnect the current root", async () => {
  const runtime = await import(`${pathToFileURL(path.join(coreRoot, "runtime/enhancer.js")).href}?contract=observer-token`);
  const previousObserver = globalThis.MutationObserver;
  const disconnected = [];
...
await test("a reusable enhancer never claims the global document", async () => {
  const runtime = await import(`${pathToFileURL(path.join(coreRoot, "runtime/enhancer.js")).href}?contract=scoped`);
  ...
  try {
    const enhancer = runtime.createEnhancer([{ name: "scoped", enhance: () => { enhanced += 1; } }]);
...
await test("createController delegates updates and optional behavior cleanup", async () => {
  const runtime = await import(`${pathToFileURL(path.join(coreRoot, "runtime/core.js")).href}?contract=controller`);
...
await test("fonts and icons stay optional", () => {
  ...
  assert.match(fs.readFileSync(path.join(coreRoot, "fonts/geist-sans.css"), "utf8"), /@font-face\b/);
  assert.match(fs.readFileSync(path.join(coreRoot, "fonts/geist-mono.css"), "utf8"), /@font-face\b/);

  const sourceIcons = fs.readdirSync(path.join(root, registry.canonicalAssets.icons)).filter((name) => name.endsWith(".svg")).sort();
  const packagedIcons = fs.readdirSync(path.join(iconsRoot, "icons")).filter((name) => name.endsWith(".svg")).sort();
  ...
  assert.match(fs.readFileSync(path.join(coreRoot, "licenses/GEIST-OFL.txt"), "utf8"), /SIL OPEN FONT LICENSE Version 1\.1/);
  assert.match(fs.readFileSync(path.join(coreRoot, "licenses/LUCIDE-LICENSE.txt"), "utf8"), /ISC License/);
  const iconExport = iconsPackage.exports["./*.svg"];
  assert.equal(iconExport, "./icons/*.svg");
  packagedIcons.forEach((filename) => {
    assertLocalPath(iconsRoot, `icon export ${filename}`, iconExport.replace("*", filename.replace(/\.svg$/, "")));
  });
  for (const exportName of ["./licenses/LUCIDE-LICENSE.txt", "./LICENSE", "./manifest.json", "./checksums.json"]) {
...
await test("all generated CSS references resolve inside the core package", () => {
  for (const relativePath of walkFiles(path.join(coreRoot, "css")).filter((filename) => filename.endsWith(".css"))) {
  ...
  for (const relativePath of walkFiles(path.join(coreRoot, "fonts")).filter((filename) => filename.endsWith(".css"))) {
...
await test("each component stylesheet entry contains exactly its declared dependency closure", () => {
  for (const component of registry.components) {
    ...
    const imports = Array.from(source.matchAll(/@import\s+"\.\/components\/([^"/]+)\.css";/g), (match) => match[1]);
...
await test("packaged SVG icons contain no executable or remote content", () => {
  for (const filename of fs.readdirSync(path.join(iconsRoot, "icons"))) {
    ...
    assert(!/(?:href|xlink:href)\s*=\s*['\"](?:https?:|\/\/|javascript:|data:text\/html)/i.test(source), `${filename}: remote or executable reference is forbidden`);
  }
});
