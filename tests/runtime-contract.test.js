...
test("combobox filters and selects with keys, calendar moves selection, carousel changes state", () => {
    const combo = node("section", { "data-ui-combobox": "" }), input = node("input", { "data-ui-part": "input", role: "combobox" }), list = node("div", { "data-ui-part": "listbox", role: "listbox" }), empty = node("p", { "data-ui-part": "empty" }), alpha = node("div", { role: "option", id: "alpha", "aria-selected": "true" }, "Alpha"), beta = node("div", { role: "option", id: "beta" }, "Beta"); list.hidden = true; empty.hidden = true; list.append(alpha, beta); combo.append(input, list, empty); mount(combo); assert.equal(input.getAttribute("aria-activedescendant"), "alpha"); input.value = "none"; fire(input, "input"); assert.equal(empty.hidden, false); input.value = "bet"; fire(input, "input"); assert.equal(empty.hidden, true); assert.equal(alpha.hidden, true); assert.equal(input.getAttribute("aria-activedescendant"), "beta"); key(input, "Enter"); assert.equal(input.value, "Beta"); assert.equal(list.hidden, true);
