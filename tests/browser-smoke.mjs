...
async function component(slug) { await load(`${base}/snippets/${slug}.html`); }
...
await component("navigation-menu"); await page.click("#navigation-menu-products-trigger"); assert.equal(await page.$eval("#navigation-menu-products-panel", (node) => node.hidden), false);
await component("select"); await page.click("#select-timezone"); await page.keyboard.press("End"); await page.keyboard.press("Enter"); assert.equal(await page.$eval('input[name="timezone"]', (node) => node.value), "europe-london");
await component("combobox"); await page.focus("#combobox-framework"); await page.keyboard.down("Control"); await page.keyboard.press("A"); await page.keyboard.up("Control"); await page.keyboard.type("Sve"); assert.equal(await page.$eval("#combobox-framework-svelte", (node) => node.dataset.uiActive), "true"); await page.mouse.click(4, 4); assert.equal(await page.$eval("#combobox-framework-list", (node) => node.hidden), true); assert.equal(await page.$eval("#combobox-framework", (node) => node.getAttribute("aria-expanded")), "false"); await page.focus("#combobox-framework"); await page.keyboard.press("Enter"); assert.equal(await page.$eval("#combobox-framework", (node) => node.value), "Svelte");
await clickAndAssert("dialog", '[data-ui-part="trigger"]', '[role="dialog"]');
await clickAndAssert("alert-dialog", '[data-ui-part="trigger"]', '[role="alertdialog"]');
await clickAndAssert("drawer", '[data-ui-part="trigger"]', "#drawer-settings");
await clickAndAssert("sheet", '[data-ui-part="trigger"]', "#sheet-notifications");
...
await component("toast"); await page.click('[data-ui-part="trigger"]'); assert.equal(await page.$eval('[data-ui-part="toast"]', (node) => node.hidden), false); await page.click('[data-ui-close]'); assert.equal(await page.$eval('[data-ui-part="toast"]', (node) => node.hidden), true);
