...
const neutralSteps = ["000", "050", "100", "200", "300", "400", "500", "600", "700", "800", "900", "950", "1000"];
...
await test("solid palettes expose the exact requested steps and reference anchors", () => {
  const expectedHexes = {
    neutral: ["#ffffff", "#f4f4f4", "#e8e8e8", "#cfcfcf", "#b3b3b3", "#979797", "#7a7a7a", "#5f5f5f", "#464646", "#303030", "#1d1d1d", "#151515", "#0a0a0a"],
    red: ["#fff1f0", "#ffe1e0", "#ffbfbe", "#ff9695", "#f66a6d", "#dc414b", "#b32a35", "#871d26", "#5e1419", "#3d090d", "#2e0709"],
    amber: ["#fff3e0", "#ffe5b4", "#f7c962", "#deac2e", "#be9011", "#9a740c", "#785a0a", "#594309", "#3d2e08", "#261c04", "#1c1404"],
    green: ["#eaf8e9", "#d6efd7", "#b0d9b7", "#85c193", "#58a770", "#2f8a52", "#196d3c", "#0e512b", "#02381b", "#00230e", "#001a09"]
  };
...
await test("authored CSS uses RGB as its only color notation", () => {
  const cssFiles = [
    ...fs.readdirSync(path.join(root, "library"), { recursive: true })
      .filter((file) => file.endsWith(".css"))
      .map((file) => path.join(root, "library", file)),
    ...fs.readdirSync(path.join(root, "docs/css"), { recursive: true })
      .filter((file) => file.endsWith(".css"))
  ...
  for (const file of cssFiles) {
    ...
    assert.doesNotMatch(source, /#[\da-f]{3,8}\b|\b(?:oklch|oklab|hsl|hsla|rgba)\(/i, `${path.relative(root, file)} mixes color notations`);
  }

  for (const [name, color] of baseDeclarations) {
    if (name.startsWith("--color-")) assert.match(color, /^rgb\(/, `${name} must use rgb()`);
  }
  assert.doesNotMatch(tokenSource, /\btransparent\b|color-mix\(in (?!srgb\b)/i, "semantic colors must remain in the RGB system");
});
...
await test("the shared L* tone curve is monotonic, symmetric, and dense at both endpoints", () => {
  ...
  neutralTones.slice(1).forEach((current, index) => {
    assert(current < neutralTones[index], `${neutralSteps[index + 1]} must be darker than ${neutralSteps[index]}`);
  });
  ...
  assert(Math.abs(lightHalfStep - darkHalfStep) <= 0.3, "050 and 950 endpoint spacing must feel symmetric");
  assert(lightHalfStep < neutralTones[2] - neutralTones[3], "the light endpoint must be denser than a full interior step");
  assert(darkHalfStep < neutralTones.at(-4) - neutralTones.at(-3), "the dark endpoint must be denser than a full interior step");
});
...
await test("paired semantic color steps meet the normal-text contrast floor", () => {
  for (const family of solidFamilies) {
    ...
    assert(lightContrast >= 4.5, `${family} 600 on 050 has only ${lightContrast.toFixed(2)}:1 contrast`);
    assert(darkContrast >= 4.5, `${family} 400 on 950 has only ${darkContrast.toFixed(2)}:1 contrast`);
  }
});
...
await test("directional neutral alpha ramps cover every available step and reproduce their solids", () => {
  ...
  for (const step of neutralSteps.slice(1)) {
    ...
    result.forEach((channel, index) => {
      assert(Math.abs(channel - target[index]) <= 0.001, `alpha-light-${step} does not reproduce neutral-${step}`);
    });
  }
  ...
  for (const step of neutralSteps.slice(0, -1)) {
    ...
    const target = solid("neutral", step);
    result.forEach((channel, index) => {
      assert(Math.abs(channel - target[index]) <= 0.001, `alpha-dark-${step} does not reproduce neutral-${step}`);
    });
  }
});
...
await test("chromatic alpha ramps retain their tuned stops and add source-colored transparent endpoints", () => {
  ...
  for (const family of colorFamilies) {
    ...
    assert.deepEqual(start.rgb, first.rgb, `${family}-000 must retain the light alpha source color`);
    assert.deepEqual(end.rgb, last.rgb, `${family}-1000 must retain the dark alpha source color`);
    assert(start.rgb.some((channel) => channel > 0) && end.rgb.some((channel) => channel > 0), `${family} endpoints must not collapse to transparent black`);
  }
});

await test("semantic color references resolve without using deprecated neutral alpha aliases", () => {
  const known = new Set([
    ...baseDeclarations.keys(),
    ...declarations(tokenSource).keys()
  ]);
  for (const match of tokenSource.matchAll(/var\((--[\w-]+)/g)) {
    assert(known.has(match[1]), `unresolved semantic reference ${match[1]}`);
  }
  assert(!/var\(--color-alpha-neutral-/.test(tokenSource), "semantic roles must use directional neutral alpha tokens");
});
