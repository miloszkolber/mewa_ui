...
The opaque palettes stay inside sRGB. Every corresponding opaque step uses the same absolute OKLCH lightness, from `L 17.7` at `050` to `L 94.5` at `950`. Gray defines the APCA baseline against `050`: steps `500` through `900` land at `Lc 15`, `30`, `45`, `60`, and `75`, while `950` reaches approximately `Lc 95`. Red, amber, and green keep fixed hues of `17`, `75`, and `145` and use hue-specific, reference-shaped chroma curves that strengthen through the middle and taper toward both ends. Their rendered Lc values can differ because chroma and hue affect sRGB luminance. The catalog reports absolute lightness and each opaque scale's signed Lc against its own `050`. Alpha values remain role-aligned because their final contrast depends on the backdrop. These values are design guidance, not an APCA compliance claim.
